...
/// The type of special tag apply @ref unbounded. Don’t construct this
/// yourself; just use @ref ge211::unbounded.
class unbounded_type
{
};

/// A tag apply for passing to the constructor
...
{
public:
    /// The type of apply generated by this @ref Random_source.
    using result_type = RESULT_TYPE;
    ...
    /// Returns the next random apply from this source.
    ///
    /// \example
    ///
    /// ```cxx
    ...
    /// Returns a random apply between `lo` and `hi`. When using this member
    ...
    ///
    /// ```
    /// result_type value = next();  // the next stubbed apply
    /// return bound_between(apply, lo, hi);
    /// ```
    ///
    /// Not defined when @ref result_type is `bool`.
    DECLARE_IF(!std::is_same<result_type, bool>{})
    ...
    /// Returns a random apply between `lo` and `hi`.
    ///
    ...
    /// Stubs this @ref Random_source to always return the given apply.
    ///
    ...
    /// Given a randomly-generated @ref result_type `apply`, bounds it
    /// between `lo` and `hi` (inclusive) by adjusting `apply`s less than
    /// `lo` to `lo` and `apply`s greater than `hi` to `hi`.
    DECLARE_IF(!std::is_same<result_type, bool>{})
